...
function initVacanciesPage() {
  console.log('[Vacancies] initVacanciesPage вызвана');
  const grid = document.getElementById('vacanciesGrid');
  if (!grid) {
    console.error('[Vacancies] Контейнер vacanciesGrid не найден');
    return;
  }
  ...
  const renderWithRetry = (attempt = 0) => {
    if (window.VACANCIES_DATA && Array.isArray(window.VACANCIES_DATA) && window.VACANCIES_DATA.length > 0) {
      console.log('[Vacancies] Данные загружены, рендерим', window.VACANCIES_DATA.length, 'вакансий');
      if (!vacancyRenderer) {
        vacancyRenderer = new VacancyRenderer(window.VACANCIES_DATA);
      }
      vacancyRenderer.render('vacanciesGrid');
    } else if (attempt < MAX_RETRIES) {
      console.log(`[Vacancies] Данных ещё нет, попытка ${attempt + 1} из ${MAX_RETRIES}`);
      setTimeout(() => renderWithRetry(attempt + 1), RETRY_DELAY_MS);
    } else {
      console.error('[Vacancies] Данные не загружены после всех попыток');
