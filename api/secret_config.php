...
// [FIX-P1] В продакшне без SMTP — падаем с 500, а не работаем с фейковыми данными.
// В dev-окружении (APP_DEBUG=true) — fallback для удобства разработки.
if (empty($smtp_user) || empty($smtp_pass) || empty($admin_emails_string)) {
    $isDebug = filter_var(
    ...
    // Dev-окружение: fallback для удобства разработки
    error_log("WARNING: Missing SMTP credentials. Using dev fallback.");
...
// ---- Лимиты ----
define('MAX_FILE_SIZE',  (int)(getenv('MAX_FILE_SIZE')  ?: 10 * 1024 * 1024));
define('MAX_TOTAL_SIZE', (int)(getenv('MAX_TOTAL_SIZE') ?: 30 * 1024 * 1024));
